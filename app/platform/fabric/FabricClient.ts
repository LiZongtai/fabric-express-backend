 ...
 */
 export class FabricClient {
	...
	async initialize(persistence) {
		...
		console.debug('Client configuration [%s]  ...', this.config.getNetworkId());
		...
		if (channels) {
			this.status = true;
			console.debug('Client channels >> %j', channels.channels);
			// Initialize channel network information from Discover
			for (const channel of channels.channels) {
				console.debug('Initializing channel ', channel.channel_id);
				try {
					await this.initializeNewChannel(channel.channel_id);
					console.debug('Initialized channel >> %s', channel.channel_id);
				} catch (error) {
					console.error('Failed to initialize new channel: ', channel.channel_id);
				}
			}
		} else if (persistence) {
			console.info('********* call to initializeDetachClient **********');
			this.initializeDetachClient(persistence);
		} else {
			console.error('Not found any channels');
		}
	}
	...
	async initializeDetachClient(persistence) {
		console.debug('initializeDetachClient', this.config.getNetworkId());
		const network_config = this.config.getConfig();
		const peers = this.config.getPeersConfig();

		console.info('initializeDetachClient, network config) ', network_config);
		console.info(
			'************************************* initializeDetachClient *************************************************'
		);
		console.info('Error :', explorerError.ERROR_1009);
		console.info('Info : ', explorerError.MESSAGE_1001);
		console.info(
			'************************************** initializeDetachClient ************************************************'
		);
	...
	async initializeNewChannel(channel_name) {
		...
		console.debug('Genesis Block for client [%s]', this.network_id);
	...
	async initializeChannelFromDiscover(channel_name) {
		...
		if (discover_results && 'peers_by_org' in discover_results) {
			for (const org in discover_results.peers_by_org) {
				console.info('Discovered', org, discover_results.peers_by_org[org].peers);
			}
		}
