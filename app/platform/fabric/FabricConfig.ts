...
export class FabricConfig {
	...
	isFabricCaEnabled() {
		...
		console.info('Fabric CA: Enabled');
		return true;
	}
	 ...
	 */
	getTls() {
		console.info('config.client.tlsEnable ', this.config.client.tlsEnable);
	...
	getAdminUser() {
		if (
			...
			console.error('client.adminCredential.id is undefined');
			return null;
		}
	...
	getAdminPassword() {
		if (
			...
			console.error('client.adminCredential.password is undefined');
			return null;
		}
	...
	getDefaultChannel() {
		...
		for (const x in this.config.channels) {
			...
			console.info('FabricConfig, this.config.channels ', x);
			if (x) {
				defChannel = x;
			}
		}
	...
	getOrgSignedCertPem() {
		...
		if (
			...
			console.error('Not found signedCert configuration');
	...
	getOrgAdminPrivateKeyPem() {
		...
		if (
			...
			console.error('Not found adminPrivateKey configuration');
			throw new ExplorerError(explorerError.ERROR_2015);
		}
	...
	getPeerTlsCACertsPem(peer) {
		...
		if (
			...
			console.error(`Not found tlsCACerts configuration: ${peer.url}`);
			return '';
		}
