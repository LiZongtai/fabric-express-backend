...
export class FabricEvent {
	...
	async createChannelEventHub(channel_name) {
		...
		try {
			...
			console.info('Successfully created channel event hub for [%s]', channel_name);
		} catch (error) {
			console.error(`Failed to add block listener for ${channel_name}`);
		}
	}
	...
	async disconnectChannelEventHub(channel_name) {
		console.debug('disconnectChannelEventHub(' + channel_name + ')');
		const listener = FabricEvent.channelEventHubs.get(channel_name);
		const network = await this.client.fabricGateway.gateway.getNetwork(
			channel_name
		);
	 ...
	 */
	disconnectEventHubs() {
		console.debug('disconnectEventHubs()');
		...
		for (const listenerEntry of FabricEvent.channelEventHubs) {
			...
			if (status) {
				...
				console.debug('disconnectEventHubs(), no connection found ', channel_name);
			}
		}
	}
}
