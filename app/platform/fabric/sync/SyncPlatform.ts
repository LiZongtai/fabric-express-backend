...
export class SyncPlatform {
	...
	async initialize(args: string | any[]) {
		...
		console.info(explorerError.MESSAGE_1002, this.network_id, this.network_name);

		console.debug('Blocks synch interval time >> %s', this.blocksSyncTime);
		...
		await (async function updateNetworkConfig(sync) {
			console.info('Updating the client network and other details to DB');
			const res = await sync.syncService.synchNetworkConfigToDB(sync.client);
			if (!res) {
				console.error('Failed to update network config to DB');
			}
