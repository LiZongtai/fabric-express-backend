...
export class SyncServices {
	...
	async syncBlocks(client, channel_name, noDiscovery) {
		...
		if (!channelInfo) {
			console.info(`syncBlocks: Failed to retrieve channelInfo >> ${channel_name}`);
			return;
		}
		const synch_key = `${network_id}_${channel_name}`;
		console.info(`syncBlocks: Start >> ${synch_key}`);
		if (this.synchInProcess.includes(synch_key)) {
			console.info(`syncBlocks: Block sync in process for >> ${synch_key}`);
			return;
		}
		...
		if (results) {
			for (const result of results) {
				...
				try {
					...
					console.error(`Failed to process Block # ${result.missing_id}`);
				}
			}
		} else {
			console.debug('Missing blocks not found for %s', channel_name);
		}
		const index = this.synchInProcess.indexOf(synch_key);
		this.synchInProcess.splice(index, 1);
		console.info(`syncBlocks: Finish >> ${synch_key}`);
	}
	...
	async processBlockEvent(client, block, noDiscovery) {
		...
		console.debug('New Block  >>>>>>> %j', block.header.number);
		const channel_genesis_hash = client.getChannelGenHash(channel_name);
		...
		if (!channel_genesis_hash) {
			...
			console.warn('Insert discovered new channel', channel_name);
		...
		for (let txIndex = 0; txIndex < txLen; txIndex++) {
			...
			console.debug('saveTransaction ', res);
		}

		// Insert block
		console.info('block_row.blocknum ', block_row.blocknum);
		const successSaveBlock = await this.persistence
			.getCrudService()
			.saveBlock(network_id, block_row);
		console.debug('result of SaveBlock ', successSaveBlock);
